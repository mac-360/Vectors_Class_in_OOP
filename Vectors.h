...
public:
    ...
    Vectors(std::initializer_list<Type> elements) : vecptr(nullptr), TotalCapacity(elements.size()), NoofElemets(elements.size()) {
    ...
    Type* emplace(Type* ref, Args&&... args) {
    ...
    void erase(int start, int end) {
        if (start < 0 || start >= NoofElemets || end <= start || end > NoofElemets) {
